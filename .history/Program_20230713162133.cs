...
public class Program
{
    public static void Main()
    {
        ...
        using (var fileStream = new FileStream("legalLength.txt", FileMode.Create))
        {
            fileStream.SetLength(fileSizeInBytes);
        }
        // 通过使用 FileStream 创建文件并将其大小设置为所需大小
        using (var fileStream = new FileStream("illegalLength.txt", FileMode.Create))
        {
            fileStream.SetLength(fileSizeInBytes+1);
        }
